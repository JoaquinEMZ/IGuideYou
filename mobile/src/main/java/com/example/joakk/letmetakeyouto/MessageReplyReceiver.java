...
public class MessageReplyReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (MyMessagingService_Auto.REPLY_ACTION.equals(intent.getAction())) {
            int conversationId = intent.getIntExtra(MyMessagingService_Auto.CONVERSATION_ID, -1);
    ...
    private CharSequence getMessageText(Intent intent) {
        ...
        if (remoteInput != null) {
            return remoteInput.getCharSequence(MyMessagingService_Auto.EXTRA_VOICE_REPLY);
        }
        return null;
    }
}
